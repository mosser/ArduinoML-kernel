...
exports.joinToNode = exports.expandToNode = void 0;
const generator_node_1 = require("./generator-node");
const template_string_1 = require("./template-string");
const NEWLINE = Object.freeze(new generator_node_1.NewLineNode());
const UNDEFINED_SEGMENT = Object.freeze(new generator_node_1.CompositeGeneratorNode());
/**
...
function findIndentationAndTemplateStructure(staticParts) {
    const lines = staticParts.join('_').split(generator_node_1.EOL);
...
function splitTemplateLinesAndMergeWithSubstitions(staticParts, substitutions, { indentation, omitFirstLine, omitLastLine, trimLastLine }) {
    ...
    staticParts.forEach((part, i) => {
        splitAndMerged.push(...part.split(generator_node_1.EOL).map((e, j) => j === 0 || e.length < indentation ? e : e.substring(indentation)).reduce(
...
function composeFinalGeneratorNode(splitAndMerged) {
    ...
    const result = splitAndMerged.reduce((res, segment, i) => segment === UNDEFINED_SEGMENT
        ...
        : segment === NEWLINE
            ? {
                indent: '',
                node: (i === 0 || (0, generator_node_1.isNewLineNode)(splitAndMerged[i - 1]) || typeof splitAndMerged[i - 1] === 'string')
            ...
            const indent = res.indent === '' && typeof segment === 'string' && segment.length !== 0 ? ''.padStart(segment.length - segment.trimLeft().length) : res.indent;
            return {
                indent,
        ...
        })(), { indent: '', node: new generator_node_1.CompositeGeneratorNode() });
    return result.node;
}
function joinToNode(iterable, toString = String, { prefix, suffix, appendNewLineIfNotEmpty } = {}) {
    return reduceWithIsLast(iterable, (node, it, i, isLast) => {
        node !== null && node !== void 0 ? node : (node = new generator_node_1.CompositeGeneratorNode());
        return node
